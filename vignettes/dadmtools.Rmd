...
* prefix ()
* key_field_grskey_table (gr_skey)
...
The updated resultant table will include the field from the following table.
...
A row is added to `batch_add_fields_to_resultant.csv`. The values for each of the columns are updated, with the value in (parentheses).
...
Once the parameters are updated, the code chunk is run. Note, if the template tiff used is not the default, that needs to be defined in the paramater list, and added to the call `get_gr_skey_extent`.
