...
#' A table with gr_skey needs to exist prior to running this function
...

{
  ...
  for (row in 1:nrow(in_file)) {
    ...
    overwrite_flds      <- as.logical(gsub("[[:space:]]","",toupper(in_file[row, "overwrite_fields"])))
    incl_prefix         <- as.logical(gsub("[[:space:]]","",toupper(in_file[row, "include_prefix"])))
    new_rslt            <- gsub("[[:space:]]","",tolower(in_file[row, "new_resultant_name"]))
    gr_skey_tbl         <- gsub("[[:space:]]","",tolower(in_file[row, "gr_skey_table"]))
    att_tbl             <- gsub("[[:space:]]","",tolower(in_file[row, "attribute_table"]))
    rslt_tbl            <- gsub("[[:space:]]","",tolower(in_file[row, "current_resultant_table"]))
    incl_flds           <- gsub("[[:space:]]","",tolower(in_file[row, "included_fields"]))
    upd_flds            <- gsub("[[:space:]]","",tolower(in_file[row, "update_field_names"]))
    prefix              <- gsub("[[:space:]]","",tolower(in_file[row, "prefix"]))
    key_rslt            <- gsub("[[:space:]]","",tolower(in_file[row, "key_resultant_tbl"]))
    key_grskey          <- gsub("[[:space:]]","",tolower(in_file[row, "key_grskey_tbl"]))
    key_att             <- gsub("[[:space:]]","",tolower(in_file[row, "key_join_tbl"]))
