...
for (row in 1:nrow(in_file)) {
  ...
  if(dadmtools::is_blank(prefix)){prefix <- NULL}
  if(dadmtools::is_blank(upd_flds)){upd_flds <- NULL}else{
    upd_flds <- unlist(strsplit(upd_flds, ","))
  }
  if(dadmtools::is_blank(notes)){notes <- NULL}
  if(dadmtools::is_blank(att_tbl)){att_tbl <- NULL}
  if(dadmtools::is_blank(key_att)){key_att <- NULL}


  if(is.null(prefix) && incl_prefix){print("ERROR: Prefix not provided while include_prefix is TRUE")
    stop()}

  if(is.null(key_att) && !is.null(att_tbl)){print("ERROR: key_join_tbl not provided while attribute_table is populated")
    stop()}
  ...
  if(length(incl_flds) == length(upd_flds)) {print(TRUE)}
