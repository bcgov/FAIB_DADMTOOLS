...
#' @param datatype defaults to 'Int32L' 
...
rasterize_sf <- function(
  ...
  if(is.null(in_src) && is.null(pg_conn_param)){
    stop("Must have one in_src or pg_conn_param populated")
  }
  if(!is.null(in_src) && !is.null(pg_conn_param)){
    stop("Must have only one in_src or pg_conn_param populated")
  }
  if(!is.null(in_src)){
    src <- in_src
  }
