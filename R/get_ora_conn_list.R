...
#' @param serviceName  oracle service name. Default is oracle keyring value.
...
get_ora_conn_list<-function(host=NULL,user=NULL,serviceName=NULL,server=NULL, password=NULL,port='1521'){
  ...
  connList <- list(host=host,user=user,serviceName=serviceName,server=server,password=password,port=port)
  return(connList)}
