...

{
  ...
  if(!is.null(join_attribute_table)){
    ...
    }}else{join_attribute_table_schema <- NULL
  ...
  ####VERIFIY FIELDS TO BE ADDEDS TO RESULTANT
  if (!is.null(updated_field_names) ) {
    if(length(updated_field_names) == length(fields_to_include)){ print("field lengths match")
    }else{
  ...
  if(!is.null(join_attribute_table)){
    ...
    join_table_fields_all <- dadmtools::sql_to_df(join_table_fields_query,pg_conn_param)$column_name}else{
  ...
  }else{print("fields_to_include fields are not all in join table ")}
  ...
  if(!include_prefix){
    if(is.null(updated_field_names)){
      ...
      if (length(indices_to_remove > 0)){fields_to_include <- fields_to_include[-indices_to_remove]}
      if (length(indices_to_remove > 0)){updated_field_names <- updated_field_names[-indices_to_remove]}}
   }
  ...
  if(overwrite_resultant_table){
    ...
    if(output_tbl_exists){print(glue("ERROR: Output resultant table already exists"))
      stop()  }else{
