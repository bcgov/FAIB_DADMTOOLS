                      ...
                      where        = '')
{
  ...
  geom_name <- faib_dadm_tools::get_pg_geom_name(src_schema, src_tbl, pg_conn_param)
  ...
  if(is.null(flds_to_keep) || flds_to_keep == '' || is.na(flds_to_keep)) {
    ...
    sqlstmt <- (faib_dadm_tools::sql_to_df(query, pg_conn_param))$sqlstmt
  ...
  faib_dadm_tools::run_sql_r(glue('DROP TABLE IF EXISTS {dst_schema}.{dst_tbl} CASCADE;'), pg_conn_param)
  faib_dadm_tools::run_sql_r(glue('CREATE TABLE {dst_schema}.{dst_tbl} AS {sqlstmt};'), pg_conn_param)
  ## add primary key sequential integer
  faib_dadm_tools::run_sql_r(glue("ALTER TABLE {dst_schema}.{dst_tbl} ADD COLUMN {pk} int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY;"), pg_conn_param)
