...
getGeomNamePG <- function(schema, tablename, pg_conn_param ) {
  geom_name <- (getTableQueryPG(paste0("select column_name
        FROM information_schema.columns As c
            WHERE table_schema = '",schema,"' and table_name = '",tablename,"' and udt_name = 'geometry'"), pg_conn_param))$column_name
  return(geom_name)
}
